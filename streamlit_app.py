import streamlit as st
from streamlit_gsheets import GSheetsConnection
import pandas as pd

st.set_page_config(page_title="Tabungan Target", layout="centered")
st.title("🎯 Tabungan Target")

# Detail kredensial kamu
creds = {
    "type": "service_account",
    "project_id": "aplikasi-tabungan-target",
    "private_key_id": "a2b70a22d9e3d0172073001fb082cda9476c7b59",
    "private_key": "-----BEGIN PRIVATE KEY-----\\Sq/AtN90SmUAtfI+6VPvyGqAQ4ZX7pJZhLHdUhmeJ0UBNLyahlL3lPb2pTabuDAw\SU3wDCu9v64NdTl9SfBPlZpLP092KNrbnInZEqmzyRo6b5QOIAB9F86iXVHffd5b\OAifW9EoQpb8RxGyRMrlikh1uQKBgQDTNNv/d//X5RqjF6jj11D4GK11h7NUcZ9y\n96R9053hCIURQ3seirevxCwt1XeFJoeJcN2M6p2hDpnBdhDgmr5ZA/CsSlC0EOlS\wcZt01x2T2jvior8A3CHVpuaEgiRkUehTFS0u4w2EwyiXXBqHCajil0F2VnaIAjt\\nKEb1p8vzP0gYEQ3adUV5Fv8Z\n-----END PRIVATE KEY-----\n",
    "client_email": "admin-tabungan@aplikasi-tabungan-target.iam.gserviceaccount.com"
}

spreadsheet_url = "https://docs.google.com/spreadsheets/d/1DeucSufj0CH87BKRg1YqHosfdtCO-olHkgktAT9QBUk/edit"

try:
    # Memanggil koneksi langsung dengan kredensial
    conn = st.connection("gsheets", type=GSheetsConnection, **creds)
    
    with st.form("form_nabung"):
        nama = st.text_input("Nama Target")
        nominal = st.number_input("Nominal", step=1000)
        tgl = st.date_input("Tanggal")
        submit = st.form_submit_button("Simpan ke Google Sheets")

    if submit:
        # Baca data lama
        df = conn.read(spreadsheet=spreadsheet_url)
        # Gabung data baru
        new_row = pd.DataFrame([{"Nama Target": nama, "Nominal": nominal, "Tanggal": tgl.strftime("%d/%m/%Y")}])
        df_update = pd.concat([df, new_row], ignore_index=True)
        # Update ke Sheets
        conn.update(spreadsheet=spreadsheet_url, data=df_update)
        st.success("Mantap Rif! Data berhasil disimpan.")

    st.divider()
    st.dataframe(conn.read(spreadsheet=spreadsheet_url), use_container_width=True)

except Exception as e:
    st.error(f"Sistem masih terkendala: {e}")
